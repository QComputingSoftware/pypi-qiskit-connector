...
def test_connector_returns_backend_pkg():
    try:
        backend = connector()
...
def test_get_plan_value_error_no_plan_pkg(monkeypatch):
...
def test_get_plan_value_error_missing_name_pkg(monkeypatch):
    from qiskit_connector import _get_plan
...
def test_save_account_missing_creds_pkg(monkeypatch):
...
def test_list_backends_pkg(monkeypatch):
...
def test_connector_no_backend_pkg(monkeypatch):
    class MockService:
...
def test_save_account_success_pkg(monkeypatch):
    from qiskit_connector import save_account
...
def test_connector_lists_qpus_pkg(monkeypatch):
    class MockBackend:
        def __init__(self, name): self.name = name
        version = "1.0"
        num_qubits = 7
